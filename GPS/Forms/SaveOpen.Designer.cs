...
namespace AgOpenGPS
{
    ...
    public partial class FormGPS
    {
        ...
        public void GELoadABKMLExplorer(string _fileName)
        {
            int existAB = 0;
            ...
            try
            {
                ...
                for (int i = 0; i < elemList.Count; i++)
                {
                    ...
                    if (numberSets.Length > 2)
                    {
                        ...
                        vec2 bndPt = new vec2(easting, norting);
                        ABLine.lineArr[numb].origin = bndPt;
                        ABLine.lineArr[numb].heading = Convert.ToDouble(desclist[i].InnerText);
            ...
            catch (System.IO.FileNotFoundException)
            {
                ...
                Console.WriteLine("Bad or Missing AB-KML file");
            }
        }
        ...
        public void FileSaveABKML()
        {
            ...
            for (int i = 0; i < ABLine.lineArr.Count; i++)
            {
                ...
                kml.WriteElementString("name", ABLine.lineArr[i].Name);
                kml.WriteElementString("description", ABLine.lineArr[i].heading.ToString());
                kml.WriteStartElement("Style");
                ...
                linePts = pn.GetLocalToWSG84_KML(ABLine.lineArr[i].origin.easting - (Math.Sin(ABLine.lineArr[i].heading) * ABLine.abLength),
                     ABLine.lineArr[i].origin.northing - (Math.Cos(ABLine.lineArr[i].heading) * ABLine.abLength));
                linePts += pn.GetLocalToWSG84_KML(ABLine.lineArr[i].origin.easting, ABLine.lineArr[i].origin.northing);
                linePts += pn.GetLocalToWSG84_KML(ABLine.lineArr[i].origin.easting + (Math.Sin(ABLine.lineArr[i].heading) * ABLine.abLength),
                    ABLine.lineArr[i].origin.northing + (Math.Cos(ABLine.lineArr[i].heading) * ABLine.abLength));
